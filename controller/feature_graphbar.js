...
const getGraphBar1 = conn => {
    return new Promise((resolve,reject) => {
        conn.query(`SELECT DISTINCT Source,Count(Source) as jumlahNama from book_gabungan WHERE book IN (SELECT book FROM book_gabungan WHERE book LIKE 'book1') ORDER BY Count(Source) DESC Limit 10`,(err,result) => {
            if(err){
                reject(err);
            }else{
                resolve(result);
